...
class RangeAnglePublisher(object):
    def __init__(self, query_hz=30):
        self.angle_publisher = rospy.Publisher('distance_angle_measurement', SegwayTilt, queue_size=5)
        self.query_hz = query_hz
        self.query_timer = None

        self.ddynrec = DDynamicReconfigure("RangeAngleSensorDynRec")
        self.ddynrec.add_variable("sensor_height", "sensor height from pivot point", 0.05, -20, 20)
        self.ddynrec.add_variable("sensor_xdist", "sensor lateral distance from pivot point", 0.05, -20, 20)
        self.ddynrec.add_variable("sensor_angle", "sensor angle from ground (rads, 0=down)", 0.05, -20, 20)
    ...
    def start(self):
        self.query_timer = rospy.Timer(rospy.Duration(1/self.query_hz), self.publish_data)
